def sumarNumeros(a,b):
    ...
    if isinstance(a,int) and isinstance(b,int):     #isinstance (obj,type) -> valida si el objeto insertado es del tipo que indicamos en este caso a o b y que sean enteros
        resultado = a + b                                                   
        mensaje = print(f"El resultado es {resultado} SW")  #print(f"") permite concatenar variables entre llaves en lugar de "mensaje "+variable+"continuacion mensaje"                  
        return mensaje                              #retornamos el mensaje                       
    else:                                                                   
        mensaje = print("Ambos parámetros deben ser números enteros.")      
        return mensaje                              #retornamos el mensaje                         
